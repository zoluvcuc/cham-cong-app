...
export default function AdminUsersPage() {
  ...
  const handleResetPassword = async (userId: string) => {
    const newPass = prompt("Nhập mật khẩu mới muốn cấp cho nhân viên này (Ít nhất 6 ký tự):", "123456");
    if (!newPass) return;
    ...
    try {
      ...
      if (data.success) {
        alert("✅ Đã đổi mật khẩu thành công! Hãy báo nhân viên đăng nhập bằng mật khẩu mới: " + newPass);
      } else {
        ...
        {employees.map((emp) => (
          <div key={emp.id} className="bg-white p-4 rounded-xl shadow-sm border border-gray-200 hover:shadow-md transition-shadow flex flex-col">
            ...
            <div className="mt-auto">
              ...
              {/* Nút Cấp lại mật khẩu (Luôn hiện ở dưới cùng) */}
